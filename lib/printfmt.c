...
printnum_helper(void (*putch)(int, void*), void *putdat,
	 unsigned long long num, unsigned base, int width, int padc, int state, int origin)
{
	if ((char)padc != '-'){
		if (num >= base){
			printnum_helper(putch, putdat, num / base, base, width - 1, padc, state, origin);
		}else {
		...
		if (num >= base) {
			printnum_helper(putch, putdat, num / base, base, width - 1, padc, state + 1, origin);
		}
		// then print this (the least significant) digit
		putch("0123456789abcdef"[num % base], putdat);
		if (width == origin) {
	 ...
	 unsigned long long num, unsigned base, int width, int padc)
{
	...
	// // then print this (the least significant) digit
	// putch("0123456789abcdef"[num % base], putdat);
	printnum_helper(putch, putdat, num, base, width, padc, 0, width);
}
