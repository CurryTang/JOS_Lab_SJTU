...
mem_init(void)
{
	...
	// Lab4: change to original 4K map
	boot_map_region(kern_pgdir, KERNBASE, 1+(~KERNBASE), 0, PTE_P | PTE_W);
...
page_alloc(int alloc_flags)
{
	...
	if(!page_free_list){
		//cprintf("No enough space in page_alloc\n");
		return NULL;
	}
	struct PageInfo* nextfree = NULL;
	nextfree = page_free_list;
	page_free_list = page_free_list->pp_link;
	nextfree->pp_link = NULL;
	char *addr = page2kva(nextfree);
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref == 0 || !pp->pp_link){
		...
		panic("This page can not be freed at this moment");
	}
}
...
{
	// Fill this function in
	struct PageInfo* new_alloc_page = NULL;
	pte_t *pgtab;
	pde_t* pde;
	pde = &pgdir[PDX(va)];
	if(*pde & PTE_P){
	...
	if(!create){
		...
		new_alloc_page = page_alloc(ALLOC_ZERO);
		if(!new_alloc_page){
			return NULL;
		}
		new_alloc_page->pp_ref++;
		*pde = page2pa(new_alloc_page) | PTE_P | PTE_W | PTE_U;
		pgtab = (pte_t *)(KADDR(PTE_ADDR(*pde)));
		return &pgtab[PTX(va)];
	}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	for(i = 0;i < n;i++){
		...
		// 	cprintf("memory allocation fail\n");
		// 	return;
		// }
		// if (*pte & PTE_P){
		// 	panic("Remap, map already exists");
		// }
		*pte = pa | perm | PTE_P;
		va += PGSIZE;
		pa += PGSIZE;
...
boot_map_region_large(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
	...
	pte_t *pte;
	uint32_t i = 0;
	uint32_t n = size / PTSIZE;
	for(i = 0;i < n;i++){
		// here we can not use pgdir_walk!!!!
		pte = &pgdir[PDX(va+i)];
		// if (!pte){
		// 	// memory allocation fail
...
page_remove(pde_t *pgdir, void *va)
{
	// Fill this function in
	pte_t *ptentry;
	struct PageInfo* ppage = page_lookup(pgdir, va, &ptentry);
	if(ppage){
		page_decref(ppage);
		if(*ptentry){
			*ptentry = 0;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	perm |= PTE_P;
	for(;start_addr < end_addr;start_addr=ROUNDDOWN(start_addr+PGSIZE, PGSIZE)){
		if (start_addr >= ULIM) {
			user_mem_check_addr = start_addr;
			return -E_FAULT;
		}
		pte_t *corr_pte = pgdir_walk(env->env_pgdir, (void *)start_addr, 0);
		if (!corr_pte) {
			user_mem_check_addr = start_addr;
			return -E_FAULT;
		}
