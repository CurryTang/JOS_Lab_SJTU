...
trap_init(void)
{
	...
	extern void ENTRY_IRQ_TIMER();
	extern void ENTRY_IRQ_KBD();
	extern void ENTRY_IRQ_2();
	extern void ENTRY_IRQ_3();
	extern void ENTRY_IRQ_SERIAL();
	extern void ENTRY_IRQ_5();
	extern void ENTRY_IRQ_6();
	extern void ENTRY_IRQ_SPURIOUS();
	...
	extern void ENTRY_IRQ_IDE();
	extern void ENTRY_IRQ_15();
	extern void ENTRY_IRQ_ERROR();
...
trap_init_percpu(void)
{
	...
	gdt[(GD_TSS0 >> 3) + index ] = SEG16(STS_T32A, (uint32_t) (&(thiscpu->cpu_ts)),
...
trap_dispatch(struct Trapframe *tf)
{
	...
	switch(tf->tf_trapno){
		...
		default:{
			//do nothing go the the part below
		}
	}
...
page_fault_handler(struct Trapframe *tf)
{
	...
	if (curenv->env_pgfault_upcall) {
		...
		if((uint32_t)(UXSTACKTOP - tf->tf_esp <= PGSIZE) && (uint32_t)(UXSTACKTOP - tf->tf_esp) >= 1){
